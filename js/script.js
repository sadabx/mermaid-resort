...
if (roomsContainer && typeof roomsData !== 'undefined') {
    ...
    <div class="room-card fade-in-up scroll-reveal" style="transition-delay: ${(index % 3) * 0.05}s" data-room="${room.name}" data-price="${room.price}">
      <div class="room-card-media">
        ...
        <div class="gallery-nav">
          ...
          <button class="gallery-prev"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
          <button class="gallery-next"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
          ` : ''}
        </div>
      </div>
      <div class="room-card-info">
        <h3 class="room-card-title">${room.name}</h3>
        <p class="room-card-price">৳${room.price.toLocaleString()}</p>
        <div class="room-card-features">
          ${room.features.join(' <span>•</span> ')}
        </div>
      </div>
    </div>
  `).join('');
  ...
  if (roomPickerDropdown) {
    roomsData.forEach(room => {
      ...
      item.innerHTML = `
        <div>
          <p class="room-picker-item-name">${room.name}</p>
          <p class="room-picker-item-features">${room.features.join(' · ')}</p>
        </div>
        <span class="room-picker-item-price">৳${room.price.toLocaleString()}</span>
      `;
...
modal.addEventListener('click', (e) => { 
  if (e.target === modal || e.target.closest('.modal-backdrop') || e.target.classList.contains('modal-wrapper')) closeModal(); 
});
  ...
  contactForm.addEventListener('submit', function(e) {
    ...
    .then(async (response) => {
      ...
      if (response.status == 200) {
        ...
        contactSubmitBtn.style.backgroundColor = '#16a34a'; // green-600 color
      } else {
